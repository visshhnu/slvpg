...
export async function onRequestPost({ request, env }) {
  ...
  await env.DB.prepare(`
    INSERT INTO payments (rent_ledger_id, resident_id, amount, payment_mode, payment_type, collected_by, reference_note)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(
    rent_ledger_id || null, resident_id, amount,
    payment_mode || 'cash', payment_type || 'rent',
    session.name, reference_note || null
  ).run();

  // If this payment is against a specific month's rent ledger row, update it
  if (rent_ledger_id) {
